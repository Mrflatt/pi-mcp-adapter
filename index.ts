...
function installMcpAdapter(pi: ExtensionAPI, options: McpAdapterOptions) {
  ...
  if (earlyConfig.settings?.scriptMode === true) {
    (pi.registerTool as (tool: unknown) => unknown)({
      name: "mcp_script",
      label: "MCP Script",
      description: "Run a trusted JavaScript MCP script to chain and filter tool calls. Use the flat tools.<prefixedToolName>(args) proxy; for names with hyphens or other non-identifier characters, use tools[\"server_tool-name\"](args).",
